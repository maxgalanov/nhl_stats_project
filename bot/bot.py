...
async def get_players(message):
    ...
    await bot.send_message(
        message.chat.id, "Вратарь или полевой игрок?", reply_markup=markup
    )
...
async def get_stats(message):
    ...
    async with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        if data["player"] == "goalies_agg":
            df["perc_goals"] = round(
                df["shotsAgainst"] / (df["shotsAgainst"] + df["goalsAgainst"]) * 100, 2
            )
    ...
    ans += (
        "Игр в стартовом составе: " + str(int(res["gamesStarted"])) + "\n"
        if data["player"] == "goalies_agg"
        else ""
    )
    ans += (
        "Игр на ноль: " + str(int(res["shutouts"])) + "\n"
        if data["player"] == "goalies_agg"
        else ""
    )
    ans += (
        "Процент отраженных бросков: " + str(res["perc_goals"]) + "\n"
        if data["player"] == "goalies_agg"
        else ""
    )
    ans += (
        "Среднее время на льду: "
        + str(int(res["toi"] / res["gamesCNT"]) // 60)
        + " минут, "
        + str(int(res["toi"]) % 60)
        + " секунд"
        + "\n"
    ...
    ## add for all types
    ans += (
        "Очков: " + str(int(res["points"])) + "\n"
        if data["player"] == "skaters_agg"
        else ""
    )
    ans += "Голов: " + str(int(res["goals"])) + "\n"
    ans += (
        "Показатель полезности: " + str(int(res["plusMinus"])) + "\n"
        if data["player"] == "skaters_agg"
        else ""
    )
    ...
    if data["player"] == "skaters_agg":
        await bot.send_message(
            message.chat.id,
            f"""Более подробную информацию можете посмотреть в нашем [дашборде по игрокам](https://datalens.yandex/xqnhz02g6x6ml?tab=lD&playersfio_d7s6={data['name'].split(' ')[0]}%20{data['name'].split(' ')[1]})""",
            parse_mode="MarkdownV2",
        )
    elif data["player"] == "goalies_agg":
        await bot.send_message(
            message.chat.id,
            f"""Более подробную информацию можете посмотреть в нашем [дашборде по игрокам](https://datalens.yandex/xqnhz02g6x6ml?tab=24G&playersfio_imb3={data['name'].split(' ')[0]}%20{data['name'].split(' ')[1]})""",
            parse_mode="MarkdownV2",
        )
...
async def get_datalens(message):
    ...
    await bot.send_message(
        message.chat.id, "Выберите вкладку дэшборда", reply_markup=markup
    )
